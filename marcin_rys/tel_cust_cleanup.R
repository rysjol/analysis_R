...
# # read 'Telecom_customer_churn.csv'
...
test_reduced <- z$train[seq(from = 1, to =  nrow(z$test), by = 19),]



# drop each non-numeric, non-logical (to have churn) and non-integer column and save to 'tel_cust_num'
drop.cols <- c()
for (i in 1:ncol(telco_analysis)) {
  colname <- colnames(telco_analysis[i])
  if(sapply(telco_analysis[i], class) == "numeric" || sapply(telco_analysis[i], class) == "integer" || sapply(telco_analysis[i], class) == "logical"){
    next
  } else {
...
#preparing train and test sets from cleaned dataset
seed = 7
ratio = 0.8
z_num <- train_test_split(tel_cust_num$Customer_ID, seed, tel_cust_num, ratio)
train <- z_num$train
test <- z_num$test

train_reduced_num <- z_num$train[seq(from = 1, to =  nrow(z_num$train), by = 70),]
test_reduced_num <- z_num$train[seq(from = 1, to =  nrow(z_num$test), by = 19),]